...
# ─── Client-next (React + Vite, port 5174) ──────────────

# Install client-next dependencies
client-next-install:
    cd {{project_root}}/apps/client-next && bun install

# Start client-next dev server
client-next:
    cd {{project_root}}/apps/client-next && bun run dev

# Build client-next for production
client-next-build:
    cd {{project_root}}/apps/client-next && bun run build

# Typecheck client-next
client-next-typecheck:
    cd {{project_root}}/apps/client-next && bun run typecheck

# ─── Install ─────────────────────────────────────────────

# Install all dependencies (server + client + client-next)
install: server-install client-install client-next-install
